...
colores_altura <- c("#C8E6A7", "#FFF176", "#66BB6A",
                    "#D9884E", "#8B4513", "#9b5de5")
...
pal_generica <- colorNumeric("YlGnBu", domain = c(0, 15))
...
precipitaciones %>%
  ...
  addRectangles(
    lng1 = ~ lon - 0.05 / 2, lat1 = ~ lat - 0.05 / 2,
    lng2 = ~ lon + 0.05 / 2, lat2 = ~ lat + 0.05 / 2,
    fillColor = ~ pal_generica(precip_diarias),
    color = "black",
    weight = 0.5,
    fillOpacity = 0.8,
...
# Paleta de color para precipitaciones diarias
pal_generica <- colorNumeric("YlGnBu", domain = c(0, 6000))
...
precipitaciones %>%
  ...
  addRectangles(
    lng1 = ~ lon - 0.05 / 2, lat1 = ~ lat - 0.05 / 2,
    lng2 = ~ lon + 0.05 / 2, lat2 = ~ lat + 0.05 / 2,
    fillColor = ~ pal_generica(precip_anual),
    color = "black",
    weight = 0.5,
    fillOpacity = 0.8,
    label = ~ paste("Precipitación anual:", round(precip_anual, 2), "mm")
  ) %>%
  addLegend(
    "bottomright",
    pal = pal_generica,
...
chirps_prom_norte <- precipitaciones_norte %>%
...
chirps_prom_santos <- precipitaciones_santos %>%
  group_by(year, lon, lat) %>%
  summarise(precip_anual = sum(chirps), .groups = "drop") %>%
  group_by(lon, lat) %>%
  summarise(
...
tmap_arrange(map_norte, ncol = 1)
tmap_arrange(map_santos, ncol = 1)
