...
# Dataframe con resultados de errores y parámetros asociados a cada simulación
resultado_errores = pd.read_parquet('resultados_prueba2.parquet')
resultado_errores["cov"] = resultado_errores["cov"].apply(lambda x: np.array(ast.literal_eval(x)))

# Expandir covariables espaciales en dataframe
covariables_espaciales = np.vstack(resultado_errores["cov"])
columnas_espaciales = ["G0", "G1", "G2", "G3", "G12", "G22", "G32"]

dt_cov = pd.DataFrame(covariables_espaciales, columns = columnas_espaciales)

resultado_errores = pd.concat([resultado_errores, dt_cov], axis=1)
resultado_errores = resultado_errores.drop(columns=["cov"])
