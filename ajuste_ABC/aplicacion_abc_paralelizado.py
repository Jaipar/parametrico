...
def corrida_simulacion(real, cov_original, m, nsites, indicadoras_tiempo, dist_mat):
    filas = []
    nsimul = 15000
...
# Número de cores que quieres usar
n_cores = 10

# Creamos un pool de workers
with Pool(processes=n_cores) as pool:
    # Map la función a 8 ejecuciones
    resultados = pool.starmap(
        corrida_simulacion,
        [(real, cov_original, m, nsites, indicadoras_tiempo, dist_mat) for _ in range(n_cores)]
    )
...
pd.DataFrame(filas_totales).to_parquet('resultados_abc_los_santos.parquet')
