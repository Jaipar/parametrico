...
def model_prior_cov_tiempo():
  covariables_tiempo = previa_covariables(4)
...
def simular_bern(n, p, nsites=1):
    ...
    p_col = p[:, None]
    X1 = np.random.binomial(1, p_col, size=(n, nsites))
    return X1 / p_col

# Versión de función para asignar p_t con función logística
def simular_pt(doy, b0, b1sin, b1cos, b2sin, b2cos):
    pt = expit(
        b0
        + b1sin * np.sin(2 * np.pi * doy / 365.25)
        + b1cos * np.cos(2 * np.pi * doy / 365.25)
        + b2sin * np.sin(4 * np.pi * doy / 365.25)
        + b2cos * np.cos(4 * np.pi * doy / 365.25)
    )
    return pt
...
def model_prior_cov_tiempo(ncov):
  y_train_cov_tiempo = previa_covariables(ncov)
  return(y_train_cov_tiempo)
...
# Variable de dias del año
dias = datos_santos_train[(datos_santos_train['lon']==ubic.iloc[0]['lon']) & (datos_santos_train['lat']==ubic.iloc[0]['lat'])]['date'].dt.dayofyear
...
def corrida_simulacion(real, cov_original, m, nsites, dias, dist_mat, seed):
    filas = []
    ...
    # Matriz para covariables temporales
    W_tiempo = np.column_stack([
        np.sin(2*np.pi*dias/365.25),
        np.cos(2*np.pi*dias/365.25),
        np.sin(4*np.pi*dias/365.25),
        np.cos(4*np.pi*dias/365.25)
        ])

    for sss in range(nsimul):
        cov = cov_original.copy()
        phi, sigma, beta3, rho = model_prior()
        y_train_gamma_auxiliar = model_prior_cov(cov)
        y_covariables_tiempo = model_prior_cov_tiempo(4)
        b0, b1sin, b1cos, b2sin, b2cos = model_prior_cov_tiempo(5)
        ...
        p_t = simular_pt(dias, b0, b1sin, b1cos, b2sin, b2cos) # m x 1
        ...
        # Procesos temporales
        covariables_tiempo = np.exp(W_tiempo @ y_covariables_tiempo).reshape(m, 1) # m x 1

        # Base para simulaciones (con todas las covariables)
        base = covariables_completo * covariables_tiempo # 1 x nsites * m x 1 = m x nsites

        # Modelos
        X_train_auxiliar_D3 = X2_auxiliar_fijo * X3_auxiliar_completo * base
        X_train_auxiliar_D4 = X2_auxiliar_completo * X3_auxiliar_completo * base
        X_train_auxiliar_D5 = X2_auxiliar_fijo * X3_auxiliar_completo * X1_auxiliar_fijo * base
        X_train_auxiliar_D6 = X2_auxiliar_completo * X3_auxiliar_completo * X1_auxiliar_fijo * base
        X_train_auxiliar_D7 = X2_auxiliar_fijo * X3_auxiliar_completo * X1_auxiliar_completo * base
        X_train_auxiliar_D8 = X2_auxiliar_completo * X3_auxiliar_completo * X1_auxiliar_completo * base
...
def main():
    ...
    with Pool(processes = n_cores) as pool:
        resultados = pool.starmap(
            corrida_simulacion,
            [
                (real, cov_original, m, nsites, dias, dist_mat, seeds[i])
...
if __name__ == "__main__":
    ...
    df.to_parquet("resultados_prueba3.parquet")
