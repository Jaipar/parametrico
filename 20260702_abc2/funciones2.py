...
def listado_percentiles(df, percentiles = [0.002, 0.005, 0.01, 0.02, 0.025, 0.04, 0.05, 0.10, 0.25]):
    cuantiles = df[["errorMAE", "errorMSE"]].quantile(percentiles)
...
def resumen_errores(df, sort_by = 'mean_errorMSE'):
    resumen = df.groupby(['modelo', 'covariables'], as_index=False).agg(
        mean_errorMAE = ('errorMAE', 'mean'),
        mean_errorMSE = ('errorMSE', 'mean'),
        sd_errorMAE = ('errorMAE', 'std'),
        sd_errorMSE = ('errorMSE', 'std'),
        min_errorMAE = ('errorMAE', 'min'),
        min_errorMSE = ('errorMSE', 'min'),
        max_errorMAE = ('errorMAE', 'max'),
        max_errorMSE = ('errorMSE', 'max'),
        n = ('modelo', 'size')
    ).sort_values(sort_by, ascending=True)
    return resumen

def mejor_k_modelo_training(df, p=0.05, columna_error='mean_errorMSE', posicion_k_ranking=0):
    df_filtrado = filtrar_df_errores_por_percentil_agrupado(df, columna_error, p)
    resumen = resumen_errores(df_filtrado, sort_by=columna_error)
    fila = resumen.iloc[posicion_k_ranking]
...
def corrida_simulacion_exp_errores(m, nsites, design_mat, dist_mat, D_elegido, M_elegido, dt_parametros, p=0.01, id_simulacion=None, datos_pivot=None, boolean_imprimir=True):
    ...
    errorMAE = dt_parametros["errorMAE"].iloc[0]
    delta, phi, sigma, beta3, rho = dt_parametros[["delta", "phi", "sigma", "beta3", "rho"]].iloc[0]
    columnas_gamma = [col for col in dt_parametros.columns if col.startswith("gamma_")]
    gamma_covariables = dt_parametros[columnas_gamma].iloc[0]

    if boolean_imprimir:
        print(f"Calibración con errorMSE:: {errorMSE:.2f}, errorMAE:: {errorMAE:.2f}")
        print(f"delta: {delta}, phi: {phi}, sigma: {sigma}, beta3: {beta3}, rho: {rho}")
        print(gamma_covariables)
